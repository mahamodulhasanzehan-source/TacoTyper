...
import { auth, signInWithGoogle, logout, saveGameStats, saveSpeedTestStats } from './services/firebase';
...
const GLOBAL_STYLES = `
  ...
  @keyframes wordPop { 0% { transform: scale(0); opacity: 0; } 60% { transform: scale(1.2); } 100% { transform: scale(1); opacity: 1; } }
  @keyframes floatUp { 0% { transform: translateY(0) scale(1); opacity: 1; } 100% { transform: translateY(-60px) scale(1.5); opacity: 0; } }
  @keyframes sparkleOut { 0% { transform: translate(0, 0) scale(1) rotate(0deg); opacity: 1; } 100% { transform: translate(var(--tx), var(--ty)) scale(0) rotate(180deg); opacity: 0; } }
  @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
  .word-enter { animation: wordPop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
  .popup-text { position: absolute; animation: floatUp 0.8s ease-out forwards; pointer-events: none; z-index: 100; font-weight: bold; text-shadow: 2px 2px 0px #000; font-family: "Press Start 2P"; white-space: nowrap; }
  .sparkle { position: absolute; width: 8px; height: 8px; border-radius: 50%; animation: sparkleOut 0.5s ease-out forwards; pointer-events: none; z-index: 90; box-shadow: 0 0 5px currentColor; }
  .animate-fade-in { animation: fadeIn 0.5s ease-out; }
  .loading-spinner { border: 4px solid #333; border-top: 4px solid #fff; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; }
`;
...
export default function App() {
  ...
  const finishSpeedTest = async (wpm: number, cpm: number, accuracy: number) => {
      ...
      setSpeedTestResult({ wpm, cpm, accuracy, comment: "Consulting the chef..." });
